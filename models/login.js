...
class Login {
  ...
  static create(mobileNumber, callback) {
    pool.query('INSERT INTO tbappuser (mobile_number) VALUES (?)', [mobileNumber], (error, result) => {
      if (error) {
        return callback(error, null);
      }
  ...
  static update(mobileNumber, callback) {
    pool.query('UPDATE tbappuser SET mobile_number = ? WHERE mobile_number = ?', [mobileNumber, mobileNumber], (error, result) => {
      if (error) {
        return callback(error, null);
      }
